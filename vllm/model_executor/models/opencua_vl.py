...
class OpenCUA_VLMultiModalProcessor(Qwen2VLMultiModalProcessor):
    ...
    def _get_prompt_updates(
        ...
        # For OpenCUA, we need to match the actual token IDs in the dummy text
        # The dummy text is generated by OpenCUA_VLDummyInputsBuilder which uses
        # tokenizer.decode([token_id]) to get the token string
        # We need to tokenize that string to get the actual token ID that appears in the prompt
        tokenizer = self.info.get_tokenizer()
        dummy_inputs = self.info.dummy_inputs
        dummy_text = dummy_inputs.get_dummy_text({"image": 1, "video": 0})
        dummy_token_ids = tokenizer.encode(dummy_text, add_special_tokens=False)
        
        # Use the actual token ID from dummy text as target_placeholder
        # This ensures we match what's actually in the prompt
        if len(dummy_token_ids) > 0:
            target_placeholder = {
                "image": dummy_token_ids[0],  # First token from dummy text
                "video": hf_config.video_token_id,  # Use config for video (no dummy text with video yet)
            }
            logger.info(
                f"Dummy text tokenizes to {dummy_token_ids}, using first token {dummy_token_ids[0]} "
                f"as target_placeholder for image (config token ID: {hf_config.image_token_id})"
            )
        else:
            # Fallback to config token IDs
            target_placeholder = {
                "image": hf_config.image_token_id,
                "video": hf_config.video_token_id,
            }
            logger.warning(
                f"Dummy text tokenizes to empty list, using config token IDs: "
